...
object ChallengeNine extends Application with ScalaPythonChallenge {
  def run() = {
    ...
    val pointSize = 8
    val colorArr = Array.fill(pointSize)(Color.red.getRGB)
    (first ++ second).grouped(2).foreach { x => image.setRGB(x.head, x.last, pointSize, pointSize, colorArr, 0, 0) }

    ImageIO.write(image, "jpg", new File(output))
    println("Done! Wrote new image to " + output)
  }
}
